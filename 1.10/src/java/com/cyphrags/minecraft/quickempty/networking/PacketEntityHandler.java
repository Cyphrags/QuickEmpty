...
public class PacketEntityHandler implements IMessageHandler
{
	...
	public IMessage onMessage(final IMessage message, final MessageContext ctx)
	{
		...
		{
			IThreadListener tl = (WorldServer) ctx.getServerHandler().playerEntity.worldObj;
			tl.addScheduledTask(new Runnable()
			{
				...
				public void run()
				{
					if(message instanceof PacketEntity)
					{
						...
						EntityPlayer player = ctx.getServerHandler().playerEntity.worldObj.getPlayerEntityByUUID(UUID.fromString(pb.playerUUID));
						Entity target = ((WorldServer) ctx.getServerHandler().playerEntity.worldObj).getEntityFromUuid(UUID.fromString(pb.targetUUID));
						if(InventoryHandler.checkDistance(player, target))
						{
							InventoryHandler.emptyOutEntity(ctx.getServerHandler().playerEntity.worldObj, player, target);
						}
					}
				}
			});
